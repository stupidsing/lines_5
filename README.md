...
(cd ~/lines_5/ &&
ANDROID_SDK_ROOT=$(tp_android_sdk_tools) ADB=$(tp_android_sdk_tools)/platform-tools/adb &&
cp ${SUITEDIR}/src/main/html/{five-in-a-row-cordova.html,*.js} www/ &&
ANDROID_SDK_ROOT=${ANDROID_SDK_ROOT} JAVA_HOME=$(tp_jdk8) PATH=${JAVA_HOME}/bin:${PATH} tp_cordova build &&
${ADB} uninstall io.cordova.lines_5 &&
${ADB} install -r platforms/android/app/build/outputs/apk/debug/app-debug.apk &&
${ADB} shell am start -n io.cordova.lines_5/.MainActivity &&
scp platforms/android/app/build/outputs/apk/debug/app-debug.apk pointless.online:/var/www/html/storey/ &&
echo DONE)

(cd ~/lines_5/ &&
ANDROID_SDK_ROOT=$(tp_android_sdk_tools) ADB=$(tp_android_sdk_tools)/platform-tools/adb &&
ANDROID_SDK_ROOT=${ANDROID_SDK_ROOT} JAVA_HOME=/usr/lib/jvm/java-8-openjdk-amd64 PATH=${JAVA_HOME}/bin:${PATH} tp_cordova build --release &&
...
ANDROID_SDK_ROOT=$(tp_android_sdk_tools) ADB=$(tp_android_sdk_tools)/platform-tools/adb &&
ANDROID_SDK_ROOT=${ANDROID_SDK_ROOT} ./gradlew :app:bundle &&
tp_android_bundletool build-apks --bundle=app/build/outputs/bundle/debug/app.aab --output=lines_5.apks &&
${ADB} uninstall io.cordova.lines_5 &&
ANDROID_SDK_ROOT=${ANDROID_SDK_ROOT} tp_android_bundletool install-apks --apks=lines_5.apks &&
echo DONE)
```
